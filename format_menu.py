...
def main(root, text, menubar):
    ...
    formatMenu.add_command(label="Изменить фон", command=objFormat.changeBg)
    formatMenu.add_command(label="Изменить Цвет Шрифта", command=objFormat.changeFg)
    formatMenu.add_cascade(label="Шрифт", underline=0, menu=fsubmenu)
    formatMenu.add_cascade(label="Размер", underline=0, menu=ssubmenu)
    formatMenu.add_command(label="Жирный", command=objFormat.bold, accelerator="Ctrl+B")
    formatMenu.add_command(label="Курсивный", command=objFormat.italic, accelerator="Ctrl+I")
    formatMenu.add_command(label="Подчеркивать", command=objFormat.underline, accelerator="Ctrl+U")
    formatMenu.add_command(label="Перечеркнутая линия", command=objFormat.overstrike, accelerator="Ctrl+T")
    formatMenu.add_command(label="Добавить Дату", command=objFormat.addDate)
